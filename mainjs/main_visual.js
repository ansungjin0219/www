    ...
    $('.gallery li span:first-of-type').delay(700).animate({top: 380, opacity:1},'slow');
    $('.gallery li span:last-of-type').delay(1200).animate({top: 470, opacity:1},'slow');
    ...
    //   }
      ...
      $('.gallery li span:nth-of-type(1)').css('top', 410).css('opacity', 0);
      $('.gallery li span:nth-of-type(2)').css('top', 500).css('opacity', 0);
      $('.gallery li span:nth-of-type(3)').css('top', 580).css('opacity', 0);
      $('.gallery .link'+cnt).find('span:nth-of-type(1)').delay(700).animate({top: 380, opacity:1},'slow');
      $('.gallery .link'+cnt).find('span:nth-of-type(2)').delay(1200).animate({top: 470, opacity:1},'slow');
      $('.gallery .link'+cnt).find('span:nth-of-type(3)').delay(1200).animate({top: 550, opacity:1},'slow');
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      $('.btn'+cnt).css('background','#fff');//자신 버튼만 불켜  this도 사용가능?
      $('.btn'+cnt).css('width','30px');
  ...
  $('.ps').click(function(){ 
     if(onoff==true){ // 타이머가 동작 중이냐??
	       clearInterval(timeonoff);   //살인마 고용 stop버튼 클릭시
		     $(this).html('<span class="hidden">stop</span><i class="fa-solid fa-play"></i>'); //js파일에서는 경로의 기준이 html파일이 기준!!
         onoff=false;   
    ...
    //왼쪽/오른쪽 버튼 처리
    $('.main_visual .btn').click(function(){
